...
export class VoipClient extends EventEmitter {
  ...
  get engine(): WasmEngine | null { return this.#engine; }
  ...
  #initVoipOnSocket = async (): Promise<void> => {
  ...
  #checkIncomingCallOffer = (node: any): void => {
    try {
      ...
      const call = new ActiveCall(
        incomingCallId,
        this.#engine!,
        this.#config.defaultDurationMs ?? 60_000,
      );
